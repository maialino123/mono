...
import { authClient, client, queryClient } from "@/shared/api";
...
export function LinkWalletButton() {
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      await client.wallet.linkWallet({
        message,
        signature,
        walletAddress: address,
        chainId: chain.id,
  ...
  return (
    <div className="flex flex-col gap-2">
      <Button
        variant="outline"
        onClick={isConnected ? handleLink : () => openConnectModal?.()}
        disabled={isPending}
      >
