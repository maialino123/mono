...
6. **Verify Gate**: Run each command from `project.md` § Commands directly. Mark `[-]` only if the command is not defined in `project.md` or not applicable (e.g., E2E when proposal says NOT REQUIRED). For all other commands, you **MUST execute and observe pass**. NEVER skip, mark as passed without running, or rationalize skipping (e.g., "needs running app"). If a required command cannot run, **STOP and ask the user** — do not proceed to reviews.
   - **Type check**
   - **Lint**
   - **Test**
   - **E2E**
...
9. **Review Fix Loop** (max 3 rounds; tool-driven; no self-assessment):
   ...
   9.3 **Re-Verify**: Re-run step 6 (Verify Gate). All must pass before continuing.
...
10. **Gate: all tasks done + verify passed** — only mark this after steps 6-9 are all complete.
...
4. **Metadata single-writer**: Only the main agent may edit `tasks.md` and `workflow.md` during parallel execution. Sub-agents report task completion + self-check logs to the main agent, who updates checkboxes and the Revision Log. **Main agent MUST NOT tick E2E tasks complete until the referenced execution output (exact command + exit code or explicit pass/fail summary + test/spec count) is present.**
5. **Sub-agent scope**: Each sub-agent receives: the track's task list, relevant specs/design refs, affected file paths, and the commands from `project.md`. It follows the Task Execution steps below for each task in its track.
...
2. **Verify after each merge**: Run all applicable commands from `project.md` § Commands after merging each track.
...
6. Run checks: in **single-track mode**, run all applicable commands from `project.md` § Commands (type check, lint, test, and E2E if applicable). In **parallel mode**, run lint + typecheck and task-scoped/targeted tests only — the full suite runs during the merge gate.
7. Record self-check log (see Author Self-Check section below): test added, commands run, deviations.
8. Mark task complete: `- [x]` (main agent updates `tasks.md` in parallel mode). **NEVER mark a task as complete if the test was written but not executed.** A task with a test file that was never run is NOT complete.
...
>
> **E2E tasks are NOT exempt.** If an E2E task exists in `tasks.md`, the E2E test MUST be both written AND executed. If the E2E test cannot run (e.g., no dev server, missing environment), **STOP and ask the user** — do not mark the task complete or proceed to reviews.
...
- Always run the commands from `project.md` § Commands directly after changes.
...
**Status**: All tasks complete and verify passed.
**Options**: N/A
